...
export default class WideColumnarColumn {
    ...
    #log!: {
        ...
        path: string;
        fullWriter: NodeJS.WritableStream;
        ivFull: string;
    };
    ...
    async #initalize() {
        ...
        this.referencer = new WideColumnarReferencer(
            `${this.table.db.options.dataConfig.path}/${this.table.db.options.dataConfig.referencePath}/${this.table.name}/${this.name}`,
            this.table.db.options.cacheConfig.limit,
            this.table.db.options.cacheConfig.referenceType,
            this,
        );
    ...
    async #readIvfromFullLog(): Promise<string> {
    ...
    async flush(data: WideColumnarData[]): Promise<void> {
        return new Promise(async (res, rej) => {
    ...
    async set(primary: WideColumnarDataType, value: WideColumnarDataType) {
        if (this.repairMode) {
    ...
    async get(primary: WideColumnarDataType) {
        if (this.repairMode) {
            console.warn("Repair mode is on, this will not be logged");
            return null;
        }
    ...
    async #get(primary: WideColumnarDataType) {
        return new Promise<WideColumnarData | null>(async (res, rej) => {
    ...
    async #fetchLine(file: string, index: number) {
    ...
    async has(primary: WideColumnarDataType) {
        if (this.repairMode) {
            console.warn("Repair mode is on, this will not be logged");
            return false;
        }
    ...
    async delete(primary: WideColumnarDataType) {
        if (this.repairMode) {
            console.warn("Repair mode is on, this will not be logged");
            return;
        }
    ...
    async #flushFile(file: string, data: WideColumnarData[]) {
        return new Promise<void>(async (res, rej) => {
            const dataToWrite = data.map((x) => {
                const encrypted = encrypt(
                    x.toString(),
    ...
    async findOne(
        ...
        if (data) return data;
        else return await this.#findOne(query);
    }
    ...
    async findMany(
        ...
        const datas = [...cacheData, ...filesData].sort(
            this.table.db.options.cacheConfig.sortFunction,
        );
        return datas;
    }
    ...
    async deleteMany(query: (data: WideColumnarData) => boolean) {
        if (this.repairMode) {
            console.warn("Repair mode is on, this will not be logged");
            return;
        }
    ...
    async #bulkDelete(data: WideColumnarData[]) {
        const queue: Record<string, WideColumnarData[]> = {};
        const reference = await this.referencer.getReference();
        for (const d of data) {
        ...
        for (const file in queue) {
            const promise = new Promise<void>(async (res, rej) => {
                ...
                const filtered = fileData.filter(
                    (x) =>
                        !queue[file].some(
                            (y) => y.primary.value === x.primary.value,
                        ),
                );
    ...
    async all(query: (data: WideColumnarData) => boolean, limit?: number) {
        if (this.repairMode) {
            console.warn("Repair mode is on, this will not be logged");
            return [];
        }
    ...
    async fullRepair() {
        ...
        for await (const line of rl) {
            ...
            if (parsedMethod === DatabaseMethod.Set) {
                ...
                const index = datas.findIndex(
                    (x) => x.primary.value === data.primary.value,
                );
                if (index !== -1) {
        ...
        while (i < j) {
            let bufferSize = "";
            while (
                bufferSize.length < this.table.db.options.cacheConfig.limit
            ) {
                const encrypted = encrypt(
                    datas[i].toString(),
                    this.table.db.options.encryptionConfig.securityKey,
